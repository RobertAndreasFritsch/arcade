...
public class Tank implements Updateable, Drawable, ProceedsInput
{
	...
	public Tank(final int player, final Battleground battleground, final BulletFactory bulletFactory, final KeyCtrl keys)
	{
		...
		if (player == 0)
		{
			...
			try
			{
				this.TankObject = Toolkit.getDefaultToolkit()
				      .getImage(new File("/res/games/tankies/img/Tankred.png").toURI().toURL());
			}
			...
			if (player == 1)
			{
				...
				{
					this.TankObject = Toolkit.getDefaultToolkit()
					      .getImage(new File("res/games/tankies/img/Tankblue.png").toURI().toURL());
				}
				catch (final IOException e)
				{
					e.printStackTrace();
				...
				if (player == 2)
				{
					...
					{
						this.TankObject = Toolkit.getDefaultToolkit()
						      .getImage(new File("res/games/tankies/img/Tankpurple.png").toURI().toURL());
					}
					catch (final IOException e)
					{
						e.printStackTrace();
					...
					if (player == 3)
					{
						...
						{
							this.TankObject = Toolkit.getDefaultToolkit()
							      .getImage(new File("res/games/tankies/img/Tankorange.png").toURI().toURL());
						}
						catch (final IOException e)
						{
							e.printStackTrace();
	...
	public void update(final long elapsed)
	{
		...
		// Kollision
		final int posx = (int) (this.TankPosX + Math.cos(Math.toRadians(this.angle)) * 16 * Math.signum(this.speed)) / 32;
		final int posy = (int) (this.TankPosY + Math.sin(Math.toRadians(this.angle)) * 16 * Math.signum(this.speed)) / 32;
