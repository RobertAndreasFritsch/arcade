...
public class WinnerControll implements Updateable, Drawable {
	...
	public WinnerControll(final Player p1, final Player p2, final Player p3, final Player p4, final Game game, final Sound sound) {
		this.P1 = p1;
		this.P2 = p2;
		this.P3 = p3;
		this.P4 = p4;
	...
	private final Thread	rotator			= new Thread() {																		// zaehler
	                                                                                                         // fuer
	                                                                                                         // rotation
	                                                                                                         // der
	                                                                                                         // end
	...
	private Color WhiteBlack() {
		if (this.ColorCounter++ % 2 == 0) { // Gerade
			return Color.WHITE;
		}
		else { // Ungerade
			return Color.BLACK;
	...
	public void draw(final Graphics2D g) {
		...
		if (this.Winner == true && Seat.P1 == this.derGewinner) {
			...
			g.drawString("Gewinner " + this.derGewinner.getName(), -(1024 / 2) - g.getFontMetrics().stringWidth("Gewinner " + this.derGewinner.getName()) / 2, 0); // p1
			g.rotate(Math.toRadians(-180));
		}
		else {
			if (this.P1.dead == true) {
				...
				g.drawString("GAME OVER " + Seat.P1.getName(), -(1024 / 2) - g.getFontMetrics().stringWidth("GAME OVER " + Seat.P1.getName()) / 2, -10); // p1
				g.rotate(Math.toRadians(-180));
			}
		}
		...
		if (this.Winner == true && Seat.P2 == this.derGewinner) {
			...
			g.setColor(this.WhiteBlack());
			g.drawString("Gewinner " + this.derGewinner.getName(), -1024 / 2 - g.getFontMetrics().stringWidth("Gewinner " + this.derGewinner.getName()) / 2, 1024 - 10);
			...
			if (this.P2.dead == true) {
				...
				g.drawString("GAME OVER " + Seat.P2.getName(), -1024 / 2 - g.getFontMetrics().stringWidth("GAME OVER " + Seat.P2.getName()) / 2, 1024 - 10); // p2
				g.rotate(Math.toRadians(-270));
			}
		}
		...
		if (this.Winner == true && Seat.P3 == this.derGewinner) {
			...
			if (this.P3.dead == true) {
				...
				g.drawString("GAME OVER " + Seat.P3.getName(), 1024 / 2 - g.getFontMetrics().stringWidth("GAME OVER " + Seat.P3.getName()) / 2, 1024 - 10); // p3
			}
		}
		...
		if (this.Winner == true && Seat.P4 == this.derGewinner) {
			...
			g.setColor(this.WhiteBlack());
			g.drawString("Gewinner " + this.derGewinner.getName(), 1024 / 2 - g.getFontMetrics().stringWidth("Gewinner " + this.derGewinner.getName()) / 2, -10);
			g.rotate(Math.toRadians(-90));
		}
		else {
			if (this.P4.dead == true) {
				...
				g.drawString("GAME OVER " + Seat.P4.getName(), 1024 / 2 - g.getFontMetrics().stringWidth("GAME OVER " + Seat.P4.getName()) / 2, -10); // p4
				g.rotate(Math.toRadians(-90));
			}
		}
		...
		if (this.P1.dead == true && this.P2.dead == true && this.P3.dead == true && this.P4.dead == true) {
			if (!this.rotator.isAlive()) { // lebt der runner? , wenn nicht
