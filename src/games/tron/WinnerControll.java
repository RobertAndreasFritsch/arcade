...
public class WinnerControll implements Updateable, Drawable {
	...
	public void draw(Graphics2D g) {
		...
		if (Winner == true && Seat.P1 == derGewinner) {
			...
			g.drawString("Gewinner " + derGewinner.getName(),
					-(1024 / 2) - ((g.getFontMetrics().stringWidth("Gewinner " + derGewinner.getName())) / 2), 0); // p1
			g.rotate(Math.toRadians(-180));
		} else {
			if (P1.dead == true) {
				g.rotate(Math.toRadians(180));
		...
		if (Winner == true && Seat.P2 == derGewinner) {
			...
			g.setColor(WhiteBlack());
			g.drawString("Gewinner " + derGewinner.getName(),
					-1024 / 2 - ((g.getFontMetrics().stringWidth("Gewinner " + derGewinner.getName())) / 2), 1024 - 10);
			...
			if (P2.dead == true) {
				...
				g.drawString("GAME OVER " + Seat.P2.getName(),
						-1024 / 2 - ((g.getFontMetrics().stringWidth("GAME OVER " + Seat.P2.getName())) / 2),
						1024 - 10); // p2
				g.rotate(Math.toRadians(-270));
			}
		}
		...
		if (Winner == true && Seat.P4 == derGewinner) {
			...
			g.setColor(WhiteBlack());
			g.drawString("Gewinner " + derGewinner.getName(),
					(1024 / 2) - ((g.getFontMetrics().stringWidth("Gewinner " + derGewinner.getName())) / 2), -10);
			g.rotate(Math.toRadians(-90));
		} else {
			if (P4.dead == true) {
				g.rotate(Math.toRadians(90));
