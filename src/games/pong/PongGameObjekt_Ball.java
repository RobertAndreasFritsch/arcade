...
public class PongGameObjekt_Ball implements Updateable, Drawable {
	 ...
	 * @throws Exception
	 */
	PongGameObjekt_Ball(final Game game, final KeyRequest KEYS, final ArrayList<Object> tiles, final PongGameObjekt_Blocker[] blockers) {
		this.game = game;
	...
	public void update(final long elapsed) {
		...
		if (this.y <= playerWest.getY() + 300 && this.y + by >= playerWest.getY() && this.x <= playerWest.getX() + 55 && this.x + bx >= playerWest.getX()) {
		...
		// Rechter Spieler
		if (this.y <= playerEast.getY() + 300 && this.y + by >= playerEast.getY() && this.x <= playerEast.getX() + 55 && this.x + bx >= playerEast.getX()) {
		...
		if (this.y <= playerNorth.getY() + 55 && this.y + by >= playerNorth.getY() && this.x <= playerNorth.getX() + 300 && this.x + bx >= playerNorth.getX()) {
		...
		// Unterer Spieler
		if (this.y <= playerSouth.getY() + 55 && this.y + by >= playerSouth.getY() && this.x <= playerSouth.getX() + 300 && this.x + bx >= playerSouth.getX()) {
		...
		for (int Z = 0; Z <= 3; Z++) {
			if (this.x + bx >= this.blockers[Z].getX() && this.x <= this.blockers[Z].getX() + b && this.y + by >= this.blockers[Z].getY() && this.y <= this.blockers[Z].getY() + l) {
		...
		if (PongGameObjekt_Ball.Zaehler > 5) {// Ballloopbrecher
			this.dx++;
			if (this.dy == 0) {
				this.dy++;
			}
	...
	public void Pkterhalten(final int pkt, final Tile playerEast, final Tile playerNorth, final Tile playerSouth, final Tile playerWest) {
		if (pkt == 1) {
			playerNorth.setPkt();
		}
		if (pkt == 2) {
	...
	public void starter(final Tile playerEast, final Tile playerNorth, final Tile playerSouth, final Tile playerWest, final int l) {
